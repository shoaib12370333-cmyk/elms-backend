...
const { assertSupportedMarketplace, getMarketplaceLocale } = require('../config/ebayMarketplaces');
...
router.get('/start', async (req, res) => {
  ...
  try {
    ...
    const selectedMarketplaceId = assertSupportedMarketplace(marketplaceId || 'EBAY_US');
    const state = issueEbayConnectState(userId, selectedMarketplaceId);
    const locale = getMarketplaceLocale(selectedMarketplaceId) || 'en-US';
