...
router.get('/start', async (req, res) => {
  ...
  try {
    ...
    const selectedMarketplaceId = String(marketplaceId || 'EBAY_US').trim().toUpperCase();
    const state = issueEbayConnectState(userId, selectedMarketplaceId);

    // eBay's OAuth authorize endpoint is shared, but the consent/sign-in
    // experience is localized with the marketplace-specific locale. This
    // preserves the marketplace selected by the user instead of silently
    // presenting the US-default experience.
    const marketplaceLocales = {
      EBAY_US: 'en-US',
      EBAY_GB: 'en-GB',
      EBAY_DE: 'de-DE',
      EBAY_FR: 'fr-FR',
    ...
    const locale = marketplaceLocales[selectedMarketplaceId] || 'en-US';
    const url = buildAuthorizationUrl(state, locale);
