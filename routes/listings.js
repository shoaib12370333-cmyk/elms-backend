...
const { publishListing, withdrawListing, reviseActiveListing, createOrGetCustomLocation } = require('../services/ebayListingService');
...
router.get('/queue', requireAuth, async (req, res) => {
  try {
    const statuses = String(req.query.statuses || 'draft,scheduled,error,publishing')
      .split(',').map((s) => s.trim()).filter(Boolean)
      .filter((s) => ['draft', 'scheduled', 'error', 'publishing'].includes(s));
    const listings = await listListingsByStatuses(req.userId, statuses);
    res.json({ success: true, listings });
  } catch (err) {
    console.error('listings queue error:', err.message);
    res.status(500).json({ success: false, error: 'Could not load the listing queue.' });
  }
});

router.get('/counts', requireAuth, async (req, res) => {
  try {
    const [draft, scheduled, error, publishing] = await Promise.all([
      countListingsByStatus(req.userId, 'draft'),
      countListingsByStatus(req.userId, 'scheduled'),
      countListingsByStatus(req.userId, 'error'),
      countListingsByStatus(req.userId, 'publishing'),
    ]);
    res.json({ success: true, counts: { draft, scheduled, error, publishing, total: draft + scheduled + error + publishing } });
  } catch (err) {
    console.error('listing counts error:', err.message);
    res.status(500).json({ success: false, error: 'Could not load listing counts.' });
  }
});
 ...
 * Applies Live Listings edits to the actual eBay Inventory API listing first,
 * then persists the same values locally. This prevents MongoDB from claiming
 * an edit succeeded when eBay rejected it.
 */
router.post('/:id/revise', requireAuth, async (req, res) => {
  const userId = req.userId;
  const listing = await getListingById(userId, req.params.id);
  if (!listing) return res.status(404).json({ success: false, error: 'Listing not found.' });
  if (listing.status !== 'published' || !listing.ebay_offer_id) {
    return res.status(400).json({ success: false, error: 'Only a published listing with an eBay offer can be revised here.' });
  }

  const title = String(req.body?.title || listing.title || '').trim();
  const sellPrice = Number(req.body?.sellPrice);
  const quantity = Number(req.body?.quantity);
  const categoryId = String(req.body?.categoryId || listing.category_id || '').trim();
  if (!title) return res.status(400).json({ success: false, error: 'Title is required.' });
  if (!Number.isFinite(sellPrice) || sellPrice <= 0) return res.status(400).json({ success: false, error: 'A valid selling price is required.' });
  if (!Number.isInteger(quantity) || quantity < 1) return res.status(400).json({ success: false, error: 'Quantity must be at least 1.' });
  ...
  if (!accountId) return res.status(400).json({ success: false, error: 'This listing has no saved eBay destination.' });
  const account = await getEbayAccountById(userId, accountId);
  if (!account) return res.status(400).json({ success: false, error: 'The eBay account for this listing is no longer connected.' });
  const refreshToken = await getEbayAccountRefreshToken(userId, accountId);
  if (!refreshToken) return res.status(400).json({ success: false, error: 'Please reconnect the eBay account before editing this live listing.' });

  const importRecord = listing.import_id ? await getImportById(userId, listing.import_id) : null;
  const product = importRecord?.product || {};
  const mergedProduct = { ...product, title, images: listing.images_customized ? listing.images : (listing.images?.length ? listing.images : (product.images || [])) };
  try {
    await reviseActiveListing(refreshToken, {
      offerId: listing.ebay_offer_id,
      sku: listing.sku,
      title,
      description: mergedProduct.description || mergedProduct.bulletPoints?.join('\n') || title,
      images: mergedProduct.images,
      aspects: mergedProduct.ebayAspects,
      sellPrice,
      quantity,
      categoryId,
    ...
    await Listing.updateOne(
      ...
      { $set: { title, sellPrice, quantity, categoryId, errorMessage: null } }
    );
    const updated = await getListingById(userId, req.params.id);
    res.json({ success: true, listing: updated });
  } catch (err) {
    console.error('revise live listing error:', err.message);
    res.status(err.statusCode || 500).json({ success: false, error: err.message || 'eBay rejected the listing update.', ebayErrors: err.ebayErrors || null });
  }
});
...
router.post('/:id/republish', requireAuth, async (req, res) => {
  ...
  if (listing.marketplace_id && sellerSettings?.marketplaceId && listing.marketplace_id !== sellerSettings.marketplaceId) {
    return res.status(409).json({ success: false, error: `Saved marketplace ${listing.marketplace_id} does not match the selected eBay account marketplace ${sellerSettings.marketplaceId}. Save the draft destination again before republishing.` });
  }
...
router.post('/:id/publish', requireAuth, async (req, res) => {
  ...
  const preloadedListing = await getListingById(userId, id);
  if (!preloadedListing) return res.status(404).json({ success: false, error: 'Listing not found.' });
  // A draft's saved destination is authoritative. The request body is only
  // used to choose a destination for legacy drafts that have none yet.
  if (!preloadedListing.ebay_account_id) {
    if (!accountId) {
      const accounts = await listEbayAccounts(userId);
      if (accounts.length === 1) accountId = accounts[0].id;
      else {
        return res.status(400).json({ success: false, error: accounts.length ? 'Please choose which eBay account to publish to.' : 'Please connect an eBay account first.' });
      }
    }
  } else {
    accountId = preloadedListing.ebay_account_id;
  }
  ...
  if (listing.ebay_account_id) accountId = listing.ebay_account_id;
  ...
  const selectedAccount = await getEbayAccountById(userId, accountId);
  if (!selectedAccount) return res.status(400).json({ success: false, error: 'The saved eBay destination is no longer connected. Please choose another account and save the draft.' });
  if (listing.marketplace_id && selectedAccount.marketplaceId && listing.marketplace_id !== selectedAccount.marketplaceId) {
    return res.status(409).json({ success: false, error: `This draft is saved for ${listing.marketplace_id}, but the selected eBay account uses ${selectedAccount.marketplaceId}. Choose that marketplace/account or save the draft destination again.` });
  }
