 ...
 * whether they bought a plan (paid) or use the free plan, and the IP + place of their last sign-in.
 * @param {Array<{id: string}>} users serialised users
 * @returns {Promise<{ users: Array, summary: { total, online, paid, free, suspended } }>}
 */
async function enrichUsers(users) {
  const [seen, bought, logins] = await Promise.all([
  ...
  const now = Date.now();
  const summary = { total: users.length, online: 0, paid: 0, free: 0, suspended: 0 };
