...
async function assertSessionActive({ userId, sid, iat }) {
  const key = sid || 'legacy:' + userId;
  let entry = cache.get(key);
  if (!entry || Date.now() - entry.at > CACHE_MS) {
    const [user, session] = await Promise.all([
      User.findById(userId, { sessionsValidFrom: 1 }).lean(),
...
module.exports = { startSession, recordFailedLogin, assertSessionActive, forgetCache, placeText };
