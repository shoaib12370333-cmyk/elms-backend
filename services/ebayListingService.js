...
async function findExistingOffer(refreshToken, sku, marketplaceId, options = {}) {
  ...
  const result = await ebayRequest(refreshToken, 'GET', `/sell/inventory/v1/offer${query}`, undefined, options);
...
async function updateExistingOffer(refreshToken, offerId, offerBody, options = {}) {
  const current = await ebayRequest(refreshToken, 'GET', `/sell/inventory/v1/offer/${encodeURIComponent(offerId)}`, undefined, options);
  const { offerId: _offerId, listing: _listing, ...editable } = current || {};
  const merged = { ...editable, ...offerBody };
  await ebayRequest(refreshToken, 'PUT', `/sell/inventory/v1/offer/${encodeURIComponent(offerId)}`, merged, options);
  return { ...merged, offerId };
}


async function uploadImagesToEbay(refreshToken, imageUrls, deadlineAt = null) {
  ...
  async function worker() {
    while (true) {
      ...
      try {
        const remaining = deadlineAt ? deadlineAt - Date.now() : 20000;
        if (remaining <= 0) {
          const err = new Error('eBay publish timed out while importing images.');
          err.code = 'EBAY_PUBLISH_TIMEOUT';
          err.statusCode = 504;
          throw err;
        }
        ...
        if (response.data?.imageUrl) uploaded[index] = response.data.imageUrl;
      } catch (err) {
        if (err.code === 'EBAY_PUBLISH_TIMEOUT') throw err;
        const ebayErrors = err.response?.data?.errors;
 ...
 */
async function publishListing({ refreshToken, product, sellPrice, quantity, categoryId, sku, sellerSettings, timeoutMs = 4 * 60 * 1000 }) {
  ...
  const inventoryItemBody = {
    ...
    product: {
      ...
      imageUrls: await uploadImagesToEbay(refreshToken, product.images || [], deadlineAt),
  ...
  await ebayRequest(
    ...
    inventoryItemBody,
    { deadlineAt, timeoutMessage: 'eBay publish timed out while creating the inventory item.' }
  );
  ...
  try {
    existingOffer = await findExistingOffer(refreshToken, finalSku, marketplaceId, { deadlineAt, timeoutMessage: 'eBay publish timed out while checking the existing offer.' });
  } catch (err) {
  ...
  if (existingOffer?.offerId) {
    ...
    if (!offerAlreadyPublished) {
      await updateExistingOffer(refreshToken, offerId, offerBody, { deadlineAt, timeoutMessage: 'eBay publish timed out while updating the offer.' });
    }
  } else {
    try {
      const offerResult = await ebayRequest(refreshToken, 'POST', '/sell/inventory/v1/offer', offerBody, { deadlineAt, timeoutMessage: 'eBay publish timed out while creating the offer.' });
      ...
      const recovered = await findExistingOffer(refreshToken, finalSku, marketplaceId, { deadlineAt, timeoutMessage: 'eBay publish timed out while recovering the offer.' });
      if (!recovered?.offerId) throw err;
      offerId = recovered.offerId;
      offerAlreadyPublished = !!recovered.listing;
      if (!offerAlreadyPublished) await updateExistingOffer(refreshToken, offerId, offerBody, { deadlineAt, timeoutMessage: 'eBay publish timed out while updating the offer.' });
    }
  }
  ...
  const publishResult = await ebayRequest(
    ...
    undefined,
    { deadlineAt, timeoutMessage: 'eBay publish timed out while publishing the offer.' }
  );
 ...
 */
async function reviseActiveListing(refreshToken, { offerId, sku, title, description, images, aspects, sellPrice, quantity, categoryId, deadlineAt = null }) {
  if (!offerId) throw new Error('An eBay offer ID is required to revise a live listing.');
  if (!sku) throw new Error('An eBay SKU is required to revise a live listing.');
  if (!Number.isFinite(Number(sellPrice)) || Number(sellPrice) <= 0) throw new Error('A valid selling price is required.');
  if (!Number.isFinite(Number(quantity)) || Number(quantity) <= 0) throw new Error('A valid quantity is required.');
  ...
  const cleanImages = Array.from(new Set((Array.isArray(images) ? images : inventoryProduct.imageUrls || [])
    .map((u) => String(u || '').trim())
    .filter((u) => /^https?:\/\//i.test(u)))).slice(0, 24);
  const inventoryUpdate = {
    ...
    product: {
      ...
      description: description || inventoryProduct.description || title || '',
      ...(cleanImages.length ? { imageUrls: cleanImages } : {}),
      ...(aspects && typeof aspects === 'object' ? { aspects: buildAspects({ ebayAspects: aspects }) } : {}),
    },
  };
  ...
  return { offerId, sku, sellPrice: Number(sellPrice), quantity: Number(quantity), categoryId: offerUpdate.categoryId || null };
}
...
module.exports = { publishListing, withdrawListing, updateOfferPrice, reviseActiveListing, fetchBusinessPolicies, createOrGetCustomLocation };
