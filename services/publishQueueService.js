...
async function processOneQueuedListing(listing) {
  ...
  try {
    ...
    if (listing.marketplace_id && sellerSettings.marketplaceId && listing.marketplace_id !== sellerSettings.marketplaceId) {
      throw new Error(`Saved marketplace ${listing.marketplace_id} does not match the connected eBay account marketplace ${sellerSettings.marketplaceId}. Save the draft destination again before publishing.`);
    }
