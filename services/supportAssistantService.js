...
const MAX_AI_REPLIES = 2; // after that a customer who is still writing gets a person
...
async function handleCustomerMessage(ticketId, { followUp = false } = {}) {
  if (!enabled()) return { action: 'skipped', reason: 'disabled' };
  ...
  if (ticket.escalated) {
    ...
    if (followUp) await alertAdmin(ticket, { urgent: ticket.urgent, reason: 'The customer replied.', followUp: true });
  ...
  let decision;
  if (SERIOUS.test(ticket.subject + ' ' + customerText)) {
    ...
    try {
      const history = thread.slice(-6).map((t) => (t.from === 'customer' ? 'Customer' : t.from === 'ai' ? 'Assistant' : 'Admin') + ': ' + String(t.text || '').slice(0, 500)).join('\n');
