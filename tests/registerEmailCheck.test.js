// POST /api/auth/register refuses junk addresses BEFORE anything is created (no account, no credits, no mail), and lets a real address
// through even when DNS is having a bad moment. The models are in-memory stand-ins; the route and usersModel are the real ones.
...
stub('services/emailService.js', {
  ...
  sendWelcomeEmail: async (m) => { mails.push(m); },
});
...
(async () => {
  // the junk that was found on the live site, and its cousins: refused, nothing created
  ...
  assert.strictEqual(store.length, 0, 'nothing was created for any of them');
  assert.strictEqual(mails.length, 0, 'and no welcome mail went out');

  // a real address is registered; a DNS outage does not stop it
  dnsMode = 'ok';
  res = await register('real.person@gmail.com', 'realperson');
  assert.strictEqual(res.statusCode, 200);
  assert.strictEqual(res.body.success, true);
  assert.strictEqual(res.body.user.email, 'real.person@gmail.com');
  dnsMode = 'down';
