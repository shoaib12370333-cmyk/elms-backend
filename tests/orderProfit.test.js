...
let orderDocs = [];
let findCalls = 0;
const chain = (result) => { const q = { populate: () => q, sort: () => q, lean: async () => result }; return q; };
const fakes = {
  './schemas/Order': { find: () => chain(orderDocs), findOne: () => chain(orderDocs[0] || null), findOneAndUpdate: async (f, u) => ({ ...orderDocs[0], ...u, _id: oid('o1'), userId: oid('u1'), toObject() { return this; } }) },
  './schemas/Listing': { find: (f) => { findCalls++; const or = f.$or; return chain(listings.filter((l) => or.some((c) => (c.sku && c.sku.$in.includes(l.sku)) || (c.ebayListingId && c.ebayListingId.$in.includes(l.ebayListingId))))); } },
  './schemas/Import': {},
};
...
const { listOrders, setBuyPrice } = require('../models/ordersModel');
