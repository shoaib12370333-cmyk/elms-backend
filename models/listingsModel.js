 ...
 */
async function createListing(userId, { importId, ebayAccountId, marketplaceId, sku, title, mainImage, images, sellPrice, markupPercent, currency, quantity, categoryId }) {
 ...
 */
async function upsertDraft(userId, { importId, ebayAccountId, marketplaceId, sku, title, mainImage, images, sellPrice, markupPercent, currency, quantity, categoryId }) {
...
async function listListingsByStatuses(userId, statuses = []) {
  ...
  return docs.map((doc) => {
    ...
    serialized.amazon_price = doc.importId?.amazonPrice ?? doc.importId?.product?.price ?? null;
