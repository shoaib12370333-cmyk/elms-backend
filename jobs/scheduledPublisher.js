...
async function runScheduledPublish() {
  ...
  for (const listing of dueListings) {
    ...
    try {
      ...
      if (listing.marketplaceId && sellerSettings.marketplaceId && listing.marketplaceId !== sellerSettings.marketplaceId) {
        throw new Error(`Saved marketplace ${listing.marketplaceId} does not match the connected eBay account marketplace ${sellerSettings.marketplaceId}. Save the draft destination again before publishing.`);
      }
      ...
      const product = {
        ...
        title: listing.title || listing.import.product.title,
        images: listing.imagesCustomized ? (listing.images || []) : (listing.images?.length ? listing.images : (listing.import.product.images || [])),
        ebayAspects: listing.import.product.ebayAspects || {},
      };
