...
- **Warnings never stop an import.** A loss at your markup (also "cannot make money at eBay prices" and a live listing that now loses), VeRO words, a slow delivery, a low rating ... are shown as warnings. A loss is an **amber** warning: the chip says "⚠ 2 warnings" (not a red "Loss"), the amount is in the panel, and it is repeated in the message after the import. What does stop an import: not connected to ELMS, not enough credits, a product that is already live / paused / scheduled / ended, and an Amazon site that does not fit the chosen store (a UK store takes amazon.co.uk).
